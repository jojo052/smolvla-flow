"""Approved exploratory weight=1.25 follow-up; historical controls are read-only."""
import argparse
import json
import os
...
def report(args):
    ...
    if manifest['protocol']!=dict(PROTOCOL,guidance_weight=1.25):raise ValueError('Protocol drift')
    ...
    for j in jobs():
        ...
        if any(q.get('guidance_weight_cap')!=1.25 for q in row['requests']):raise ValueError('Guidance drift')
    ...
    for delay in (0,100):
        ...
        groups.append(dict(mode='rtc',guidance=1.25,delay_ms=delay,success=sum(r['success'] for r in rr),episodes=40,
    ...
    old_report=json.loads((args.source/'report.json').read_text())
    controls=[g for g in old_report['groups'] if g['mode']!='rtc']
    atomic_json(args.output/'report.json',dict(complete=True,episodes=80,run_id=run_id,groups=groups,
        table_groups=controls+groups,
...
def main():
    ...
    args=p.parse_args();args.guidance=1.25
    ...
    cases=[r for r in diagnosis['rows'] if r['weight']==1.25]
    if len(cases)!=48 or not diagnosis['reference_reproduction_passed'] or not diagnosis['parameters_unchanged']:
        raise ValueError('Missing fixed-input evidence')
    if any(r['executed_coordinate_excess']>1e-5 for r in cases):raise ValueError('Unexpected diagnostic drift')
    ...
    run_id=freeze_manifest(args.output/'manifest.json',dict(protocol=dict(PROTOCOL,guidance_weight=1.25),
        diagnosis_sha256=sha256(args.diagnosis/'summary.json'),
        approved_exception='User approved exploratory closed-loop weight1.25 after 18/48 full-block envelope failures; no executed-slice overshoot in fixed-input cases. Runtime timing, finite-action and expiry gates unchanged.',
