"""Paired official T2 async/RTC1.25 test, independent from all S1 artifacts."""
import argparse
import json
import os
...
def worker(connection, args):
    try:
        import torch
        import resource
        from scripts.t2_rtc_policy import Policy
        ...
        while True:
            ...
            if method == 'predict':
                result['metrics'].update(guidance_weight_cap=1.25, model_id='official_T2',
...
def validate_identity(row, job, manifest, run_id):
    ...
    if row.get('model_id') != 'official_T2' or 'student_sha256' in row:
    ...
    for q in row['requests']:
        if q['forward_calls'] != 2 or q['rtc_calls'] != (2 if q['rtc_active'] else 0):
            raise ValueError('Two-step call count mismatch')
...
def report(output):
    ...
    atomic_json(output/'report.json', dict(complete=True, episodes=160, checks=16,
        ...
        limitations=['Eight tasks; independent saved-run initial indices 27-31, not novel-task generalization.',
                    'Cannot directly attribute differences from S1 runs on other states to distillation.']))


def main():
    p = argparse.ArgumentParser()
    for key in ('output', 'checkpoint', 'assets-dir', 'source'):
    ...
    if args.stage == 'run':
        for path in args.source.parent.rglob('*.json'):
            if path.name in ('manifest.json', 'summary.json', 'report.json'):
                continue
            try:
                r = json.loads(path.read_text())
            except (ValueError, OSError):
                continue
            if isinstance(r, dict) and ('ticks' in r or 'steps' in r) and r.get('init_index', r.get('initial_state_index')) in range(27, 32):
                hits.append(str(path))
        if hits:
            raise ValueError('Initial states already recorded: '+str(hits))
    import torch
    from scripts import run_libero_rollout as loader
    from scripts.evaluate_s1_interleaved import make_env
    from scripts.evaluate_s1_rtc import episode, state_hash
    ...
    manifest = dict(protocol=dict(PROTOCOL, flow_steps=2, guidance_weight=1.25,
                    initial_states=list(range(27, 32)), modes=['async', 'rtc']),
        tasks=tasks, model_id='official_T2', checkpoint_files=source['checkpoint_files'],
        original_manifest_sha256=sha256(args.source/'manifest.json'),
        entry_sources={str(f): sha256(f) for f in (Path(__file__), Path(__file__).with_name('t2_rtc_policy.py'))},
        inherited_sources=source['entry_sources'], execution_runtime=source['execution_runtime'],
        saved_prior_usage=hits, jobs=schedule('formal'), checks=schedule('check'))
    ...
    try:
        ...
        if not gate['passed']:
            raise ValueError('Two-step preflight failed')
        for phase in ('check', 'formal'):
            for job in schedule(phase):
                ...
                row.update(model_id='official_T2', checkpoint_files_fingerprint=fingerprint(source['checkpoint_files']))
