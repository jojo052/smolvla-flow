"""Official T5 paired evaluation with approved initial-wait semantics."""
import argparse
import json
import os
...
def worker(connection, args):
    try:
        import torch
        import resource
        from scripts.t5_rtc_policy import Policy
        ...
        while True:
            ...
            if method == 'predict':
                result['metrics'].update(guidance_weight_cap=1.25, model_id='official_T5',
...
def validate_identity(row, job, manifest, run_id):
    ...
    if row.get('model_id') != 'official_T5' or 'student_sha256' in row:
    ...
    for q in row['requests']:
        if q['forward_calls'] != 5 or q['rtc_calls'] != (5 if q['rtc_active'] else 0):
            raise ValueError('Five-step call count mismatch')
...
def main():
    ...
    manifest = dict(protocol=dict(PROTOCOL, flow_steps=5, guidance_weight=1.25,
        ...
        tasks=tasks, model_id='official_T5', checkpoint_files=source['checkpoint_files'],
        original_manifest_sha256=sha256(args.source/'manifest.json'),
        entry_sources={str(f): sha256(f) for f in (Path(__file__), Path(__file__).with_name('t5_rtc_policy.py'),
    ...
    try:
        ...
        if not gate['passed']:
            raise ValueError('Five-step preflight failed')
        for phase in ('check', 'formal'):
            for job in schedule(phase):
                ...
                row.update(model_id='official_T5', checkpoint_files_fingerprint=fingerprint(source['checkpoint_files']))
