#!/usr/bin/env python3
"""Independent synchronous S5/S2/T2/T5 evaluation using the frozen rollout body."""
import argparse
import json
import time
import types
...
def main():
    ...
    p.add_argument('--model',choices=['T2','T5','S5','S2'],required=True)
    ...
    if (args.student is None)!=(args.model in ('T2','T5')): raise ValueError('Student weights required only for students')
    ...
    steps=5 if args.model in ('S5','T5') else 2
