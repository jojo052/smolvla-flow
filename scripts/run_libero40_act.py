#!/usr/bin/env python3
"""Independent public ACT benchmark, preserving native 100-action execution."""
...
from smolvla_flow.benchmark40 import (
    SUITES, atomic_json, episode_key, fingerprint,
    freeze_manifest, markdown_report, sha256, summarize, validate_episode,
)

ROOT = Path(__file__).resolve().parents[1]
OFFICIAL_SHA256 = "ae1d034c4d30879773a1f67590194a7ce5246c8466eb46db299696bfab2bb247"
...
def parse_args(argv=None):
    ...
    p.add_argument("--config", type=Path, default=ROOT / "configs/libero40_act_v1.json")
...
def save_report(output, manifest):
    ...
    for file in sorted((output / "formal").glob("*/*/task*/init*.json")):
        ...
        rows.append(row)
    summary = summarize(rows, ["act_public"])
...
def prepare_manifest(args, config):
    ...
    sources += [Path(m.__file__) for m in (env_module, env_processor, modeling_act)]
...
def run_episode(env, loaded, task, index, phase, run_id, video_paths):
    ...
    def measured_chunk(*a, **kw):
        ...
        if tuple(chunk.shape) != (1, 100, 7) or not torch.isfinite(chunk).all().item():
            raise RuntimeError("Full prediction violates finite [1,100,7] contract")
        return chunk

    policy.predict_action_chunk = measured_chunk
    ...
    if len(prediction_times) != (len(select_times) + 99) // 100:
        raise RuntimeError("Expected exactly one prediction per native 100-action chunk")
    ...
    return {"status": "complete", "run_id": run_id, "phase": phase, "model": "act_public",
            "key": episode_key("act_public", task["suite"], task["task_id"], index, phase),
...
def execute(args):
    ...
    expected = json.loads((ROOT / "configs/libero40_act_v1.json").read_text())
    ...
    policy_config = ACTConfig.from_pretrained(args.checkpoint, local_files_only=True)
    if policy_config.n_action_steps != 100 or policy_config.chunk_size != 100 or policy_config.temporal_ensemble_coeff is not None:
        raise RuntimeError("Native ACT execution settings differ from audited configuration")
    policy_config.device = "cuda"
    policy_config.pretrained_backbone_weights = None
    policy = ACTPolicy.from_pretrained(args.checkpoint, config=policy_config, local_files_only=True, strict=True)
    policy.eval()
    pre, post = make_pre_post_processors(policy_config, args.checkpoint,
        preprocessor_overrides={"device_processor": {"device": "cuda"}},
        postprocessor_overrides={"device_processor": {"device": "cpu"}})
    loaded = SimpleNamespace(policy=policy, preprocessor=pre, postprocessor=post)
    phases = ["pilot", "formal"] if args.phase == "all" else [args.phase]
    for phase in phases:
        if phase == "formal":
            pilots = []
            for name in SUITES:
                key = episode_key("act_public", name, 0, 10, "pilot")
        ...
        for task in manifest["tasks"]:
            ...
            try:
                for idx in ([10] if phase == "pilot" else range(10)):
                    key = episode_key("act_public", task["suite"], task["task_id"], idx, phase)
