#!/usr/bin/env python3
"""S1: independent CPU environments and one strictly batch=1 GPU server.
...
_original_episode_key = episode_key
def episode_key(model, suite, task_id, index, phase="formal"):
    if phase == "validation":
        if model != "S1" or suite not in frozen.SUITES or task_id not in range(10) or index not in (11,12):
            raise ValueError("Invalid selection episode")
        return f"validation/S1/{suite}/task{task_id:02d}/init{index:02d}"
    return _original_episode_key(model,suite,task_id,index,phase)

_original_validate_episode = validate_episode
def validate_episode(row,run_id,key):
    if row["phase"] != "validation":
        return _original_validate_episode(row,run_id,key)
    if row["key"] != key or row["run_id"] != run_id or row["status"] != "complete":
        raise ValueError("Validation provenance mismatch")
    if key != episode_key(row["model"],row["suite"],row["task_id"],row["init_index"],"validation"):
        raise ValueError("Validation identity mismatch")
    copy_row=dict(row,phase="formal",init_index=0)
    copy_row["key"]=_original_episode_key("S1",row["suite"],row["task_id"],0,"formal")
    _original_validate_episode(copy_row,run_id,copy_row["key"])
        ...
        while True:
            ...
            try:
                for index in indices:
                    key = episode_key('S1', task['suite'], task['task_id'], index, phase)
                    ...
                    row = dict(status='complete', run_id=run_id, phase=phase, model='S1', key=key,
...
def run_parallel(args, manifest, loaded, output, phase, workers):
    ...
    for task in manifest['tasks']:
        ...
        pending=[]
        for index in ([10] if phase=='pilot' else ([11,12] if phase=='validation' else range(10))):
            key=episode_key('S1',task['suite'],task['task_id'],index,phase)
            ...
            if path.exists():
                ...
                if row['checkpoint_sha256']!=manifest['student_sha256']: raise ValueError('Resume weight mismatch')
                rows.append(row)
    ...
    try:
            ...
            while busy:
                ...
                for conn in ready:
                    ...
                    if kind=='complete':
                        ...
                        generators.pop(key)
                        row['checkpoint_sha256']=manifest['student_sha256']
        ...
        expected=4 if phase=='pilot' else (80 if phase=='validation' else 400)
...
def serial_pilots(args,manifest,suites,loaded):
    ...
    if args.reference:
        ...
        for field in ('protocol','checkpoint_files','tasks','sources','versions','assets_sha256','student_sha256'):
    ...
    copy['episode_key']=lambda _m,s,t,i,p:episode_key('S1',s,t,i,p)
    ...
    for task in manifest['tasks']:
        ...
        try:
            ...
            row.update(model='S1',noise_sha256=noises,normalized_actions=actions)
...
def main():
    ...
    p.add_argument('--mode',choices=['check','check-and-run','resume','validation'],default='check')
    ...
    manifest['student_sha256']=sha256(args.student)
    for task in manifest['tasks']:
        states=suites[task['suite']].get_task_init_states(task['task_id'])
        task['state_sha256']=[hashlib.sha256(x.detach().cpu().numpy().tobytes() if isinstance(x,torch.Tensor) else x.tobytes()).hexdigest() for x in states[:13]]
        if len(task['state_sha256'])!=13: raise ValueError("Missing selection states")
    ...
    if args.mode=='validation':
        run_parallel(args,manifest,loaded,args.output/'validation_run','validation',args.workers)
        return
    if args.mode!='resume':
