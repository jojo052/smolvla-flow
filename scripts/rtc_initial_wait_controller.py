"""Isolated controller variant allowing initial waiting; runtime gates unchanged.

Imported only by the explicitly approved T10 initial-wait experiment.
"""
import argparse
import copy
import hashlib
...
def episode(args,job,task,suite,policy,run_id,resources):
    ...
    try:
        ...
        timeline=InitialWaitTimeline(job['mode']);timeline.prime(initial['normalized'],initial['actions'],initial['metrics']['elapsed_s'])
...
if __name__=='__main__':
    raise SystemExit('Use evaluate_t10_rtc_initial_wait; this module is not a standalone evaluator')
