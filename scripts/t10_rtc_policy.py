"""Official T10 adapter. No student overlay; native ten-step RTC sampling."""
import hashlib
import inspect
import math
import time
...
class Policy:
    def __init__(self,args):
        ...
        self.torch=torch
        self.loaded=_load_policy(parse_args(['--checkpoint',str(args.checkpoint),'--flow-steps','10',
    ...
    def predict(self,obs,language,mode,delay_ms,prefix=None,estimated_delay=0,noise=None,diagnostic=False):
        ...
        if self.forward_calls!=10 or self.rtc_calls!=(10 if enabled else 0):
            raise ValueError('Ten-step/RTC execution path drift')
    ...
    def preflight(self,obs,language):
        ...
        for seed in (123,124):
            ...
            for value in (plain,guided):
                ...
                if len(times)!=10 or any(len(t)!=1 or abs(t[0]-(1-i/10))>1e-6 for i,t in enumerate(times)):
                    raise ValueError('Expected t=1,0.9,...,0.1; dt=-0.1')
            if plain['metrics']['forward_calls']!=10 or guided['metrics']['rtc_calls']!=10:
                raise ValueError('Expected ten Euler denoising steps')
        ...
        return dict(passed=passed,checks=results,parameter_sha256=before,
                    reason=None if passed else 'Ten-step RTC guidance is ineffective')
