"""Lazy GPU adapter, preserving native SmolVLA RTC and sampling implementations."""
import hashlib
import inspect
import math
import time
...
class Policy:
    def __init__(self,args):
        ...
        self.torch=torch
        self.loaded=_load_policy(parse_args(['--checkpoint',str(args.checkpoint),'--flow-steps','1',
            '--mode','async','--rtc-max-guidance-weight','10','--rtc-schedule','exp']),action_execution_steps=10)
        self.policy=self.loaded.policy
        names=_configure_action_only_student(self.policy);overlay(self.policy,args.student,names)
        self.policy.eval()
    ...
    def predict(self,obs,language,mode,delay_ms,prefix=None,estimated_delay=0,noise=None,diagnostic=False):
        ...
        if (enabled and self.rtc_calls!=1) or (not enabled and (self.forward_calls!=1 or self.rtc_calls!=0)):
            raise ValueError('One-step/RTC execution path drift')
    ...
    def preflight(self,obs,language):
        ...
        for seed in (123,124):
            ...
            for value in (plain,guided):
                m=value['metrics']
                if not m['timesteps'] or any(abs(t-1.)>1e-7 for times in m['timesteps'] for t in times):
                    raise ValueError('Sampling time is not the one-step t=1 point')
            if plain['metrics']['forward_calls']!=1 or guided['metrics']['rtc_calls']!=1:
                raise ValueError('Expected one Euler denoising step')
        ...
        return dict(passed=passed,checks=results,parameter_sha256=before,
                    reason=None if passed else 'One-step RTC guidance is ineffective at the actual integration point')
