"""Official T5 adapter. No student overlay; native five-step RTC sampling."""
import hashlib
import inspect
import math
import time
...
class Policy:
    def __init__(self,args):
        ...
        self.torch=torch
        self.loaded=_load_policy(parse_args(['--checkpoint',str(args.checkpoint),'--flow-steps','5',
    ...
    def predict(self,obs,language,mode,delay_ms,prefix=None,estimated_delay=0,noise=None,diagnostic=False):
        ...
        if self.forward_calls!=5 or self.rtc_calls!=(5 if enabled else 0):
            raise ValueError('Five-step/RTC execution path drift')
    ...
    def preflight(self,obs,language):
        ...
        for seed in (123,124):
            ...
            for value in (plain,guided):
                times=value['metrics']['timesteps']
                if len(times)!=5 or any(len(t)!=1 or abs(t[0]-(1-i/5))>1e-6 for i,t in enumerate(times)):
                    raise ValueError('Expected t=1,0.8,0.6,0.4,0.2; dt=-0.2')
            if plain['metrics']['forward_calls']!=5 or guided['metrics']['rtc_calls']!=5:
                raise ValueError('Expected five Euler denoising steps')
        ...
        return dict(passed=passed,checks=results,parameter_sha256=before,
                    reason=None if passed else 'Five-step RTC guidance is ineffective')
